function weights = BLT_bcs_reconstruction(PHI,t,a,b,eta)
%---------------------------------------------------------------------------------------
% The code was developed for multispectral BLT based on MT-CS algorithm which was coded by Shihao Ji
%---------------------------------------------------------------------------------------
% Input:
%   PHI: system matrix generated by NIRFAST. Cell structure, One cell for one wavelength.
%   t:   boundary measurements. Cell structure, One cell for one wavelength.
%   a,b: parameters of Gamma prior on noise variance
%   eta: threshold for stopping the algorithm (suggested value: 1e-8)
% Output:
%   weights: sparse resolution
%
if iscell(t)
    NT = length(t);
else
...
t_new = [];
for k = 1:NT
    PHI_new = [PHI_new;PHI{k}(:,index)];
    t_new = [t_new;t{k}];
end

PHI_new_diag = PHI_new'*PHI_new + diag(alpha);
PHI_new_inv = inv(PHI_new_diag);
mu_new = PHI_new_inv*PHI_new'*t_new;

% output
[N(1),M(1)] = size(PHI{1});
weights = zeros(M(1),1);
weights(index) = mu_new;
